...
app.use((req, res, next) => {
  ...
  res.setHeader("Access-Control-Allow-Methods", "GET, POST, PATCH, DELETE"); //allow http requests
...
app.use((error, req, res, next) => {
  if (req.file) {
    //added file property from multer
...
const mongoConnection = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASSWORD}@cluster0-z3zxc.mongodb.net/${process.env.DB_NAME}?retryWrites=true&w=majority`;
mongoose
  .connect(
    mongoConnection,
